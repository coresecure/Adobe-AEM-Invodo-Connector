...
<%
    ...
    long width = (long) properties.get("width",-1);
    long height = (long) properties.get("height",-1);
    if (width > 0 && !(height > 0)) {
        height = ConfigurationUtil.getHeight(videoPlayer,width);
    } else if (height > 0 && !(width > 0)) {
        width = ConfigurationUtil.getWidth(videoPlayer,height);
    } else {
        width = ConfigurationUtil.getWidth(videoPlayer);
        height = ConfigurationUtil.getHeight(videoPlayer);
    }
